...
class TrainModel:
    def __init__(self, training_parameters):
        ...
        train_dataset = TrainingDataset(
            root_dir="../data/train_dataset/sample_dir/",
        ...
        self.train_loader = DataLoader(
            train_dataset, batch_size=training_parameters["batch_size"], shuffle=True, num_workers=0
        )

        self.validation_loader = DataLoader(
            validation_dataset, batch_size=training_parameters["batch_size"], shuffle=True, num_workers=0
        )
    ...
    def preprocess_point_cloud(self, point_cloud, sample_dir):
        ...
        x_vals = np.linspace(
            Xmin,
            Xmin + (num_boxes_x * self.training_parameters["box_dimensions"][0]),
            int(num_boxes_x / (1 - self.training_parameters["box_overlap"][0])) + 1,
        )
        y_vals = np.linspace(
            Ymin,
            Ymin + (num_boxes_y * self.training_parameters["box_dimensions"][1]),
            int(num_boxes_y / (1 - self.training_parameters["box_overlap"][1])) + 1,
        )
        z_vals = np.linspace(
            Zmin,
            Zmin + (num_boxes_z * self.training_parameters["box_dimensions"][2]),
    ...
    def run_training(self):
        ...
        optimizer = optim.Adam(
            filter(lambda p: p.requires_grad, model.parameters()), lr=self.training_parameters["learning_rate"]
        )
